...
/**
 * \file    test/phpunit/MMIProductDluoFunctionalTest.php
...
/**
 * Class MMIProductDluoFunctionalTest
 *
 ...
 *
 * @package Testmmiproductdluo
 */
class MMIProductDluoFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMIProductDluo')->click();
		} else {
			// Disable the module
			$this->byHref('modMMIProductDluo')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmiproductdluo/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmiproductdluo/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmiproductdluo/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmiproductdluo/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmiproductdluowidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMIProductDluo_MMIProductDluoTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMIProductDluo_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
